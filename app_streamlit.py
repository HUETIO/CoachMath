...
# Carga de variables de entorno solo si se usa Gemini
load_dotenv()

st.set_page_config(page_title="RAG Matemáticas (Gemini/Ollama)", page_icon="🧠", layout="wide")
st.title("🧠 RAG de Matemáticas con Gemini/Ollama - Echo por Diego Sepulveda DASH(By Reaper)")
...
def load_or_create_vs(uploaded_file, emb):
...
with st.sidebar:
    ...
    modelo = st.selectbox("Modelo", ["Gemini", "Ollama"])
...
# Selección de modelo y embeddings
if modelo == "Gemini":
    ...
    # Puedes cambiar el modelo por uno más ligero si lo deseas, por ejemplo "phi3"
    llm = ChatOllama(model="llama2:7b", temperature=temperature)
...
with col1:
    ...
    if st.button("Buscar y responder", use_container_width=True):
        vs = load_or_create_vs(uploaded_pdf, emb)
